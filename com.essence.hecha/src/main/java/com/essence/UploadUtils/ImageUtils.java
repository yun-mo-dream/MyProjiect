...
public class ImageUtils {
	...
	public static void saveImageToSD(String filePath, Bitmap bitmap, int quality)
		...
		public static void saveImageToSD2(String tofilePath, String fromfilePath, int quality)
				throws IOException {
			ByteArrayOutputStream byteArrayOutputStream=BitmapUtil.getSmallBitmap(fromfilePath, 720, 1280, 250);		
			BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(tofilePath));
			byteArrayOutputStream.writeTo(bos);	
			bos.close();	
...
//	}
	...
	public static Bitmap comp(Bitmap image) {
		...
		if (baos.toByteArray().length / 2048 > 512) {// 判断如果图片大于1M,进行压缩避免在生成图片（BitmapFactory.decodeStream）时溢出
		...
		float hh = h;// 这里设置高度为100f
		float ww = w;// 这里设置宽度为100f
		int be = 1;// be=1表示不缩放
		...
		Bitmap bitmap1 = Bitmap.createScaledBitmap(bitmap, 1024, height, true);
	...
	public static Bitmap comp1(Bitmap image) {
		...
		Log.i("打印信息", "" + hh);
		int inSampleSize = 1;
		if (height > hh || width > 1024) {
			final int heightRatio = Math.round((float) height / (float) hh);
			final int widthRatio = Math.round((float) width / (float) 1024);
			inSampleSize = heightRatio < widthRatio ? heightRatio : widthRatio;
		}
		final BitmapFactory.Options options = new BitmapFactory.Options();
		options.inSampleSize = inSampleSize;

		// Bitmap bitmap1 = Bitmap.createScaledBitmap(image, 1024, hh, true);
		Bitmap bitmap1 = compressBitmap(image, 1024, hh);
		return compressImage(bitmap1);// 压缩好比例大小后再进行质量压缩
	}
